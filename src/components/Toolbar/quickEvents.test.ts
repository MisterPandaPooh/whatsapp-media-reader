...
describe('eventSpans', () => {
  it('returns one span per year for a holiday across all years', () => {
    const spans = eventSpans('passover', 'all')
  ...
  // Passover 2025 runs 04-12 → 04-19 in the table; the filter widens it by three
  ...
  it('pads a holiday by three days on each side, from the table dates', () => {
    const [span] = eventSpans('passover', 2025)
  ...
  it('spans the new year for a holiday that straddles it, padding included', () => {
    const [span] = eventSpans('hanukkah', 2024)
  ...
  // May and make "Summer" mean something the label does not say.
  it('leaves the seasons unpadded', () => {
    expect(dayOf(eventSpans('summer', 2023)[0].from)).toBe('2023-06-01')
    expect(dayOf(eventSpans('summer', 2023)[0].to)).toBe('2023-08-31')
    expect(dayOf(eventSpans('winter', 2023)[0].from)).toBe('2023-01-01')
  })
  ...
  it('keeps every year separate after padding', () => {
    const spans = [...eventSpans('passover', 'all')].sort((a, b) => a.from - b.from)
  ...
  it('builds summer as June through August', () => {
    const [span] = eventSpans('summer', 2023)
  ...
  it('builds winter as January through the end of February, leap year included', () => {
    expect(dayOf(eventSpans('winter', 2023)[0].to)).toBe('2023-02-28')
  ...
  it('labels a single year and an all-years selection differently', () => {
    expect(eventLabel('passover', 2025)).toContain('2025')
    expect(eventLabel('passover', 'all')).toMatch(/all years/i)
  })
})

describe('filtering by quick-event spans', () => {
  const media = [
    item('2024-04-25', 'passover24'), // inside Passover 2024
    item('2025-04-14', 'passover25'), // inside Passover 2025
    ...
    item('2025-04-30', 'after25'), // well clear of Passover 2025
  ...
  it('reaches three days either side of the holiday', () => {
    const kept = filteredMedia(media, { ...EMPTY_FILTERS, dateSpans: eventSpans('passover', 2025) })
    expect(kept.map((m) => m.id).sort()).toEqual(['eve25', 'passover25', 'tail25'])
  })
  ...
  it('keeps only items from the chosen year', () => {
    const kept = filteredMedia(media, { ...EMPTY_FILTERS, dateSpans: eventSpans('passover', 2025) })
    expect(kept.map((m) => m.id).sort()).toEqual(['eve25', 'passover25', 'tail25'])
    expect(kept.map((m) => m.id)).not.toContain('passover24')
  })

  it('keeps items from every year when all years are chosen', () => {
    const kept = filteredMedia(media, { ...EMPTY_FILTERS, dateSpans: eventSpans('passover', 'all') })
    expect(kept.map((m) => m.id).sort()).toEqual(['eve25', 'passover24', 'passover25', 'tail25'])
  })

  it('does not leak the gap between two spans', () => {
    // The naive "min start to max end" reading of "all Passover" would span
  ...
  it('composes with the other filter dimensions', () => {
    const kept = filteredMedia(media, {
      ...EMPTY_FILTERS,
      dateSpans: eventSpans('passover', 'all'),
      senders: ['Nobody'],
    })
    expect(kept).toEqual([])
  })
  ...
  // them would mean "every Passover, but also only in this one month".
  it('takes precedence over a single from/to range', () => {
    const kept = filteredMedia(media, {
      ...EMPTY_FILTERS,
      dateSpans: eventSpans('passover', 2025),
